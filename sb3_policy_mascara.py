...
class A2CPolicyGNNMasked(ActorCriticPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for p in self.mlp_extractor.parameters():
            p.requires_grad = False
        for p in self.action_net.parameters():
            p.requires_grad = False
    ...
    # [MOD] helper de enmascarado robusto
    @staticmethod
    def _masked_logits(logits: th.Tensor, mask: th.Tensor) -> th.Tensor:
        """
        Devuelve logits con -inf donde mask=False (True=permitido).
        """
        # Asegura tipos/shape compatibles
        mask = mask.to(device=logits.device, dtype=th.bool)
    ...
    def _logits_and_value(self, obs: Dict[str, th.Tensor]) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        B = g.shape[0]
        destino_logits = th.full((B, self.max_nodes), th.finfo(node_logits.dtype).min, device=self.device)
        for b in range(B):
    ...
    def predict(self, observation, state=None, episode_start=None, deterministic: bool = False):
        ...
        # [MOD] Consistencia device/dtype de máscaras
        ...
        # [MOD] Fallback de seguridad si toda la fila está a False
        ...
        # [MOD] Cierra explícitamente posiciones > n_valid
        #      Necesitamos n_valid otra vez; reconstruimos rápido sin repetir forward pesado
        _, n_valid = self._build_pyg_batch(obs)
    ...
    def _predict(self, observation: Any, deterministic: bool = False):
        ...
        mascara_tipo = obs["mascara_tipo"].to(device=self.device, dtype=th.bool)
        mask2_tbl    = obs["mask2_table"].to(device=self.device, dtype=th.bool)
    ...
    def evaluate_actions(self, obs: Dict[str, th.Tensor], actions: th.Tensor):
        ...
        # 2) Máscaras al device correcto y en bool (tras _logits_and_value para asegurar device real)
        mascara_tipo = obs["mascara_tipo"].to(device=tipo_logits.device, dtype=th.bool)
        ...
        # [MOD opcional] Sanity checks en debug (comenta en prod)
        # assert mascara_tipo.any(dim=1).all(), "Fila sin acciones de tipo válidas"
        # assert (cond_mask.sum(dim=1) > 0).all(), "Fila sin destinos válidos"
